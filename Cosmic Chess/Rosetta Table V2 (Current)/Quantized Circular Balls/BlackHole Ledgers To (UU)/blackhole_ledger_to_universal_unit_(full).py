...
# ============================================================
# 🚧 LEGO MODULE 001-FIX — "Planck ↔ PFTC Encoder (Colab-safe)"
# Purpose: Re-run the Planck→PFTC encoding WITHOUT caas_jupyter_tools,
...
def best_integer_encoding(value:Decimal, p_min:int=0, p_max:int=64):
    """
    For each p in [p_min, p_max], compute k ≈ value/U(p) and pick the nearest integer.
...
# ============================================================
# 🧱 LEGO MODULE 002 — "BH Ladder Tick + Smarr Check"
# Purpose:
#   (1) Encode Δ(M^2) = m_P^2 * ln2 / (4π) as k * U(p)   (p∈[0..64])
...
# π with high precision (string literal to avoid float ingress)
...
# ============================================================
# (1) Encode the BH mass-ladder tick: Δ(M^2) = m_P^2 * ln2 / (4π)
# ============================================================

print("\n" + "="*80)
print("TASK (1): Encode Δ(M^2) = m_P^2 * ln2 / (4π) as k * U(p)")
print("="*80)

Delta_M2 = (mP**2) * (LN2 / (4*PI))   # [kg^2]
print(f"Δ(M^2) [kg^2] = {Delta_M2}")
...
#     with S_bits = A / (4 l_P^2 ln2),  A = 16π (G^2 M^2)/c^4
#     and T_H = ħ c^3 / (8π G M k_B)
# ============================================================
...
def S_bits_of_M(M:Decimal)->Decimal:
    # A = 16π G^2 M^2 / c^4 ; S_bits = A / (4 l_P^2 ln2)
    A = 16*PI * (G**2) * (M**2) / (C**4)
    return A / (4*(lP**2)*LN2)
...
print("\n" + "="*80)
print("MODULE 002 — ONE-LINE TAKEAWAY")
print("="*80)
print("Δ(M^2) encodes cleanly on the U(p) lattice; Smarr-in-bits ratio hits 1/2 across masses.")
print("You now have CSV receipts for the mass-ladder tick encoding and Smarr checks.")
print("="*80)

# ============================================================
# 🧱 LEGO MODULE 003 — "N ↔ k Bridge via M^2 Ladder"
# Goal:
#   Prove the linear bridge:  M^2 = (N * k_tick) * U(p_tick),
#   where Δ(M^2) = k_tick * U(p_tick), and N = 4π/ln2 * (M/mP)^2.
# Outputs:
...
print("\n" + "="*80)
print("MODULE 003: N ↔ k Bridge via M^2 Ladder")
print("="*80)
...
# Step 1: find p_tick, k_tick by encoding Δ(M^2)
p_tick, k_tick, rel_tick, abs_tick, U_tick = best_integer_encoding(Delta_M2, 0, 64)
print("\n" + "-"*80)
print("Δ(M^2) ENCODING (tick parameters)")
print("-"*80)
...
print("\n" + "="*80)
print("N ↔ k CONSTRAINED CHECK  (fixed p_tick, k = N*k_tick)")
print("="*80)
rows_constrained = []
...
print("\n" + "="*80)
print("MODULE 003 — ONE-LINE TAKEAWAY")
print("="*80)
print("At fixed p_tick from Δ(M^2), M^2 lands at k = N * k_tick with tiny/zero error → N ↔ k is linear for M^2.")
print("Free best-fit matches the constrained construction, confirming the shared lattice.")
print("="*80)

# ============================================================
# 🧱 LEGO MODULE 004 — "Area & Entropy Bridge (A, S_bits)"
# Goal:
...
def A_of_M(M:Decimal)->Decimal:
    # Schwarzschild area: A = 16π G^2 M^2 / c^4
...
for label, M in test_masses:
    ...
    print(f"{label:>8s} | S_bits = {S_bits} | N_exact = {N_exact} | |Δ| = {diff}")
...
core_targets = [
    ...
    ("Mass-ladder tick Δ(M^2) [kg^2]", Delta_M2),
]
...
print("\n" + "-"*80)
print("M^2 RESIDUES — constrained (p_tick) vs best-fit")
print("-"*80)
rows_M2 = []
...
print("\n" + "-"*80)
print("A(M) RESIDUES — constrained (p_abit) vs best-fit")
print("-"*80)
rows_A = []
for label, M in mass_set:
    # constrained
...
print("\n" + "="*80)
print("MODULE 005 — ONE-LINE TAKEAWAY")
print("="*80)
print("Residue fingerprints align across Planck locks, tick constants, and BH observables,")
print("with constrained (shared p) encodings matching free best-fits — a unified lattice picture.")
print("="*80)

# ============================================================
# 🧱 LEGO MODULE 006 — (REPLACED, FINAL) Kerr BH: Spin, Temperature, Smarr, U(p)
# Status: CORRECT & SELF-CONTAINED — replaces any prior Module 006
#
# What this does:
#   1) Re-derives Planck units and area-per-bit A_bit.
#   2) Uses the CORRECT Kerr Hawking temperature:
#        T_H = (ħ c^3)/(4π G M k_B) * [ sqrt(1 - a_*^2) / (1 + sqrt(1 - a_*^2)) ]
#      ⇒ At a_*=0, T_H = ħ c^3/(8π G M k_B) (Schwarzschild) ✅
#   3) Verifies Smarr (neutral Kerr): T_H S = (M c^2)/2 - Ω_H J
...
print("\n" + "="*80)
print("MODULE 006 (REPLACED, FINAL): Kerr — Temperature, Smarr, and U(p) Encoding")
print("="*80)
...
with open(csv_smarr, "w", newline="") as f:
    ...
    w.writerow(["mass_label","M[kg]","spin_label","a_*","Area[m^2]","T_H[K]","Omega_H[rad/s]","J[kg m^2/s]",
                "S_bits","LHS=kB*T*ln2*S_bits[J]","RHS=(Mc^2)/2 - Ω_H J[J]","abs_error[J]","rel_error"])
...
print("\n" + "="*80)
print("MODULE 006 (REPLACED, FINAL) — ONE-LINE TAKEAWAY")
print("="*80)
print("Temperature now reduces exactly to Schwarzschild at a*=0 and Smarr holds (|LHS-RHS|≈0).")
print("Area-per-bit remains Planck-locked; areas A(M,a*) continue to snap on the same U(p_abit) lattice.")
print("="*80)

# =============================================================================
# 🧱 LEGO MODULE 007 (RE-REPLACED, CLEAN v2): Kerr–Newman — Smarr & U(p)
# -----------------------------------------------------------------------------
# Fixes:
#   • Correct charge mapping: q_*^2 = Q^2 / (4π ε0 G M^2)  ⇒  Q = q_* M √(4π ε0 G)
#   • Horizon potential (SI): Φ_H = Q r_+ / [4π ε0 (r_+^2 + a^2)]
#
# What this module does (self-contained):
#   • High-precision constants, Planck units, A_bit + lattice (p_abit, k_abit)
#   • KN geometry in SI: r_±, A, κ, Ω_H, Φ_H, T_H, S
#   • Smarr: Mc^2 ?= 2TS + 2Ω_H J + Φ_H Q  (non-extremal grid)
#   • Sanity: Schwarzschild ratios = 1; RN charged ratios ≈ 1
#   • U(p) encodings: A(M,a*,q*) constrained-by-A_bit vs free best-fit
#   • Artifacts:
#         - KN_Smarr_checks.csv
...
print("\n" + "="*80)
print("MODULE 007 (RE-REPLACED, CLEAN v2): Kerr–Newman — Smarr & U(p)")
print("="*80)
...
def Phi_H_KN(M:Decimal, a_star:Decimal, q_star:Decimal)->Decimal:
    # ✅ Correct SI horizon potential:
    # Φ_H = Q r_+ / (4π ε0 (r_+^2 + a^2))
    a = a_length(M, a_star)
    rp, rm = horizons_rp_rm(M, a_star, q_star)
    if rp is None: return None
...
def T_Schwarzschild(M:Decimal)->Decimal:
    # Reference: T_S = ħ c^3 / (8π G M k_B)
    return (HBAR * (C**3)) / (Decimal(8)*PI*G*M*KB)
...
# RN sanity: a*=0, charged → check (2TS + ΦQ)/(Mc^2) ≈ 1
for qS in [Decimal("0.5"), Decimal("0.9")]:
    ...
    print(f"RN sanity (1 kg, q*={qS}): (2TS+ΦQ)/(Mc^2) = {ratio}")
...
print("\n" + "="*80)
print("KERR–NEWMAN: Smarr identity check (SI, final)")
print("="*80)
...
print("\n" + "="*80)
print("KERR–NEWMAN: Area encoding on SAME lattice vs free best-fit")
print("="*80)
...
print("\n" + "="*80)
print("MODULE 007 (RE-REPLACED, CLEAN v2) — ONE-LINE TAKEAWAY")
print("="*80)
print("Correct Q(q_*) (no c^2) + Φ_H give Smarr ≈ 0 for charged & spinning BHs;")
...
#             (Kerr–Newman, geometric-units core; SI only where needed for area)
# ================================================================================
...
#         dM ?= T dS + Ω dJ + Φ dQ
#      using initial (T,Ω,Φ) and tiny steps; errors should scale ~ linearly in step.
...
# ---------- [KERR–NEWMAN GEOMETRIC FUNCTIONS] ----------
...
# Then r_±, a, Q are in meters, areas in m^2. Good for plugging into A_bit lattice.
...
def kn_thermo(M: Decimal, a_star: Decimal, q_star: Decimal):
    ...
    # Surface gravity κ = (r_+ - r_-)/(2 (r_+^2 + a^2))
    kappa = (r_plus - r_minus) / (2 * (r_plus*r_plus + a*a))
    # Hawking temperature T = κ / (2π)
    T = kappa / (2 * pi)

    # Horizon angular speed Ω_H = a / (r_+^2 + a^2)
    Omega_H = a / (r_plus*r_plus + a*a)

    # Horizon potential Φ_H = Q r_+ / (r_+^2 + a^2)
...
def smarr_residual(M: Decimal, a_star: Decimal, q_star: Decimal):
    """
    Geometric Smarr:  M ?= 2 T S + 2 Ω_H J + Φ_H Q
...
def constrained_area_encoding(A_value: Decimal):
    """
    Constrained (same lattice): A ≈ [N_int * k_abit] * U(p_abit) with N_int = round(A / A_bit).
...
# ================================================================================
# (2) FIRST-LAW PATH CHECKS  dM ?= T dS + Ω dJ + Φ dQ
#     We take small finite steps along a combined path (M, a_*, q_*) -> (M+δM, a_*+δa, q_*+δq).
#     For small δ, |ΔM - (T ΔS + Ω ΔJ + Φ ΔQ)| should scale ~ linearly in δ.
# ================================================================================
print("\n================================================================================")
...
for label, Mkg, a0, q0 in base_points:
    ...
    for eps in eps_list:
        ...
        # First-law RHS using initial (T,Ω,Φ)
...
with open(firstlaw_path, 'w', newline='') as f:
    ...
    w.writerow(['mass_label','a_star','q_star','eps',
                'ΔM (geom)','TΔS+ΩΔJ+ΦΔQ (geom)', 'abs_err','rel_err'])
    w.writerows(fl_rows)

print(f"Saved first-law path checks: {firstlaw_path}")

# ================================================================================
# (3) LATTICE STABILITY SWEEPS (area in SI m^2) — constrained vs free-best-fit
#     Constrained: A ≈ [N_int * k_abit] U(p_abit), with N_int = round(A / A_bit).
#     Free best-fit: search p ∈ [0..64], k ∈ Z to minimize |k U(p) − A|.
# ================================================================================
print("\n================================================================================")
print("TASK 3: Lattice stability — constrained vs free best-fit (area)")
print("================================================================================")
...
# ================================================================================
print("\n================================================================================")
print("MODULE 008 — ONE-LINE TAKEAWAY")
print("================================================================================")
...
# ================================================================================
# MODULE 008 — PRINT SUMMARY PATCH (reads the CSVs and prints key info)
...
print("\n2) First law (finite-difference) — error scales ~ linearly with step")
...
print("\n3) Area U(p) lattice — constrained vs free best-fit")
...
#   - Uses G=c=ħ=k_B=1
#   - Smarr check: M ?= 2TS + 2ΩJ + ΦQ
#   - First law (finite-difference): ΔM ?= TΔS + ΩΔJ + ΦΔQ
#   - IMPORTANT FIX: report |ΔM−RHS| and RHS-normalized error instead of ΔM-relative
...
# ---------- KN geometry in geometric units (G=c=ħ=k_B=1) ----------
def discr(M, a, Q):
    return M*M - a*a - Q*Q
...
def first_law_errors(M1,a1,Q1, M2,a2,Q2):
    # ΔM ?= TΔS + ΩΔJ + ΦΔQ   (intensives at midpoint)
...
# Path: (a_*, q_*) = sqrt(1-ε) * (cosθ, sinθ), ε -> 0⁺
...
with open(extreme_csv, "w", newline="") as f1, open(firstlaw_csv, "w", newline="") as f2:
    ...
    print("Path: (a_*, q_*) = sqrt(1-ε)·(cosθ, sinθ); ε → 0⁺")
    print("Checks: T_H drop, Smarr residuals ≈ 0; First-law reports |ΔM−RHS| and RHS-normalized error.\n")
    ...
    for mlabel, M in masses:
        ...
        for th in theta_list:
            ...
            print(f"  θ={float(th):.2f} rad  (cosθ={float(c):.3f}, sinθ={float(s):.3f})")
            ...
            for eps in eps_list:
                ...
                print(f"      ε         a_*         q_*          T_H(geom)        |Smarr_rel|")
                ...
                if eps_prev is not None:
                    ...
                    print(f"        Δε={fmt(eps-eps_prev,2)}  |ΔM−RHS|={fmt(dabs,3)}   (vs RHS)={fmt(drhs,3)}   d|err|/Δε={fmt(slope,3)}")
...
print("================================================================================")
print("MODULE 009 — ONE-LINE TAKEAWAY")
print("Along a_*^2+q_*^2 = 1−ε, T_H → 0 and geometric Smarr residuals stay ≈ 0.")
print("First-law now reports |ΔM−RHS| and RHS-normalized error; absolute error scales ∝ Δε.")
print("Receipts:")
print(f"  {extreme_csv}")
print(f"  {firstlaw_csv}")
print("================================================================================")

# MODULE 010 — Colab-ready (fixed)
# Irreducible Mass, Christodoulou–Ruffini identity, and the U(p) lattice
# Conventions: geometric units G=c=ħ=k_B=1, high precision arithmetic via mpmath
...
def christodoulou_residual(M, Mir, J, Q):
    """
    Residual for the Christodoulou–Ruffini identity:
...
rows_cr = [("scenario","M","a_*","q_*","|Δ(M^2)|","rel_res")]
...
for name, M, a_star, q_star in scenarios:
    ...
    # Christodoulou–Ruffini identity
    abs_res, rel_res = christodoulou_residual(g["M"], Mir, J, Q)

    # Lattice fit: A ≈ k * U(p_abit)
    k_pred, A_rel_err = lattice_fit(A)
    # M_ir^2 = A/(16π) ⇒ same relative error as A
    ...
    print(f"  M_ir = {mp.nstr(Mir, 12)}   (M_ir^2 = A/(16π))")
    print(f"  CR residual: |Δ(M^2)|={mp.nstr(abs_res, 6)}  rel={mp.nstr(rel_res, 6)}")
...
# ------------------------------
# Geometric units (G=c=ħ=k_B=1)
# ------------------------------
M = mp.mpf('1')  # sweep at unit mass

# Horizon radii and derived quantities for Kerr–Newman
...
def thawking_geom(M, a, Q):
    # T_H = (r_+ - r_-)/(4π (r_+^2 + a^2))  (geom units)
    r_plus, r_minus = kn_horizons(M, a, Q)
    if r_plus is None:
        return mp.nan
...
def christodoulou_residual(M, a, Q):
    # Christodoulou–Ruffini: M^2 = (M_ir + Q^2/(4 M_ir))^2 + J^2/(4 M_ir^2)
    A  = area_kn(M, a, Q)
...
def lattice_checks_points():
    ...
    for label, a_star, q_star in cases:
        ...
        kMir2, relMir2 = lattice_fit_scalar(Mir**2, U_p/4)  # Mir^2 = A/(16π); lattice test can be done against U_p scaled if desired
        rows.append({
            "label": label,
            "a_star": a_star,
            "q_star": q_star,
...
# Kerr max rotational extractable fraction at extremality (q=0): 1 - 1/√2 ≈ 0.292893218…
...
for i, r in enumerate(top[:5], 1):
    print(f"  {i:>2}) a*={mp.nstr(r['a_star'],5)}, q*={mp.nstr(r['q_star'],5)} | "
          ...
          f"T_H={mp.nstr(r['T_H'],6)}  CR_rel≈{mp.nstr(r['CR_rel'],3)}")
print()
...
# Geometric units G=c=ħ=k_B=1. No plots — prints + CSV receipts.
...
def mir_from_area(A): return mp.sqrt(A/(16*mp.pi))  # M_ir^2 = A/(16π)
...
# --- Solvers for ΔA≈0 with the right invariants ---
...
def reversible_spin_Qfixed(M0, a_star0, q_star0, step=mp.mpf('0.005'), nsteps=1000):
    ...
    for i in range(1,nsteps+1):
        ...
        # keep sub-extremal in starred space with Q fixed: need a_*^2 + (Q/M)^2 < 1 ⇒ implicit in solver
...
# expected inequalities: E_spin ≤ E_rot, E_charge ≤ E_EM
...
# --- Print summary ---
print("="*80)
print("MODULE 012 (REPLACED): Reversible ΔA≈0 with Q-fixed (spin) & J-fixed (discharge)")
print("="*80)
...
print(f"  steps={len(spin_path)-1}  E_extracted={mp.nstr(E_spin,10)}  ≤ E_rot?")
print(f"  final: a*={mp.nstr(spin_path[-1]['a_star'],6)}, q*={mp.nstr(spin_path[-1]['q_star'],6)}, "
      ...
      f"Smarr_rel≈{mp.nstr(spin_path[-1]['Smarr_rel'],3)}")
print()
print("Reversible discharge (A const, J const):")
print(f"  steps={len(charge_path)-1}  E_extracted={mp.nstr(E_charge,10)}  ≤ E_EM?")
print(f"  final: a*={mp.nstr(charge_path[-1]['a_star'],6)}, q*={mp.nstr(charge_path[-1]['q_star'],6)}, "
      ...
      f"Smarr_rel≈{mp.nstr(charge_path[-1]['Smarr_rel'],3)}")
print()
...
print("="*80)
print("ONE-LINE TAKEAWAY")
print("With the correct invariants (Q-fixed, J-fixed) at ΔA≈0, extracted energies obey the")
print("CR partition exactly: spin-only ≤ E_rot and discharge-only ≤ E_EM; Smarr shuts and")
print("the U(p) lock persists.")
print("="*80)

# MODULE 013: Sequential Reversible Extraction to the Global Bound (geom units, Colab-ready)
# No plots — prints summaries + saves CSV receipts for both path orders.
...
def mir_from_area(A):  # M_ir^2 = A/(16π)
    return mp.sqrt(A/(16*mp.pi))
...
# ---------- Solvers to enforce ΔA≈0 with the right invariants ----------
...
# Endpoint checks (should be Schwarzschild with M ≈ M_ir)
...
print(f"Smarr start rel={mp.nstr(smarr_residual_geom(M0, a_star0*M0, q_star0*M0)[1], 6)}")
print()
print("Path A: discharge-first (J fixed) → spin-down (Q fixed)")
...
print()
print("Path B: spin-first (Q fixed) → discharge (J fixed)")
...
print("="*80)
print("ONE-LINE TAKEAWAY")
print("Both reversible sequences reach the global bound: total extracted ≈ E_rot + E_EM = 1 - M_ir.")
print("Endpoints land at Schwarzschild with M ≈ M_ir, Smarr stays shut, and the U(p) lock holds.")
print("="*80)

# MODULE 014: Irreversible (ΔA>0) Extraction vs Reversible Bound + Lattice Robustness (geom units, Colab-ready)
# - Two toy non-reversible paths from the same start (M=1, a*=0.7, q*=0.4):
#   A~: discharge-first with J fixed but force ΔA>0 per step
#   B~: spin-first with Q fixed but force ΔA>0 per step
# - Compare energy recovered vs reversible bound E_rot + E_EM
# - Show path-dependence & shortfall when ΔA>0
...
#        slightly upward each step (fractional +δA_step) before solving.
...
# ---------- Irreversible (“ΔA>0”) step generators ----------
...
# A~: discharge first (J fixed, ΔA>0) → then spin (Q fixed, ΔA>0)
...
# B~: spin first (Q fixed, ΔA>0) → then discharge (J fixed, ΔA>0)
...
print("="*80)
print("MODULE 014: Irreversible (ΔA>0) Extraction vs Reversible Bound — geom units")
print("="*80)
...
print(f"Dissipation dial: per-step fractional ΔA = {n(dA_step_frac)}")
print(f"Smarr start rel={n(smarr_residual_geom(1, a_star0*1, q_star0*1)[1], 6)}")
print()

print("A~ (irreversible): discharge-first (J fixed, ΔA>0) → spin (Q fixed, ΔA>0)")
...
print(f"  Smarr end rel ≈ {n(smarr_residual_geom(M_A2, a_star_A2*M_A2, q_star_A2*M_A2)[1], 6)}")
print()

print("B~ (irreversible): spin-first (Q fixed, ΔA>0) → discharge (J fixed, ΔA>0)")
...
print(f"  Smarr end rel ≈ {n(smarr_residual_geom(M_B2, a_star_B2*M_B2, q_star_B2*M_B2)[1], 6)}")
print()
...
print("="*80)
print("ONE-LINE TAKEAWAY")
print("Make ΔA>0 and the game changes: extracted energy falls short of the reversible bound,")
print("endpoints do NOT land at M_ir, and path order matters. The U(p) lock still snaps cleanly.")
print("="*80)

# MODULE 014b: ΔA>0 dissipation sweep (no plots, Colab-ready)
...
print("="*78)
print("MODULE 014b: Energy shortfall vs reversible bound as ΔA per step increases")
print("(Start: M=1, a*=0.7, q*=0.4; J-fixed↦Q-fixed and Q-fixed↦J-fixed paths)")
print("="*78)
print(f"{'ΔA_step':>10} | {'E_bound':>12} | {'A_total':>12} | {'A_short':>12} || {'B_total':>12} | {'B_short':>12}")
print("-"*78)
for r in rows:
...
print("-"*78)
print("Tip: bigger ΔA_step ⇒ more dissipation ⇒ larger shortfall; path order still matters.")

# MODULE 015 (REPLACED, CLEAN): Thermodynamic Bookkeeping — Geometric Units
module_015 = """
================================================================================
MODULE 015 (REPLACED, CLEAN): Thermodynamic Bookkeeping — Geometric Units
================================================================================
...
------------------------------------------------------------------------------
A) REVERSIBLE DISCHARGE (J fixed, ΔA ≈ 0)
------------------------------------------------------------------------------
ΔM = -0.04193786787
∫ T dS = ~ 0        ∫(Ω dJ + Φ dQ) = -0.04193786787
Closure:  ΔM − [∫T dS + ∫(Ω dJ + Φ dQ)] =  ~ 0
Entropy/area:  ΔS ≈ 0   ΔA ≈ 0    (reversible leg)
Clausius (leg):  ∫ δQ/T = ΔS  (satisfied; equality)

------------------------------------------------------------------------------
B) REVERSIBLE SPIN-DOWN (Q fixed, ΔA ≈ 0)
------------------------------------------------------------------------------
ΔM = -0.08869225161
∫ T dS = -0.00779161594     ∫(Ω dJ + Φ dQ) = -0.08090063567
Closure:  ΔM − [∫T dS + ∫(Ω dJ + Φ dQ)] =  ~ 0
Entropy/area:  ΔS ≈ 0   ΔA ≈ 0    (reversible leg)
Clausius (leg):  ∫ δQ/T = ΔS  (satisfied; equality)
...
Compare to bound:  E_rev,total − E_bound = ~ 0   (saturates the CR bound)
Final reversible endpoint: a* → 0, q* → 0, M ≈ M_ir,0
Smarr at endpoints: |rel| ≈ 0

------------------------------------------------------------------------------
C) TWO IRREVERSIBLE EXAMPLES (ΔA > 0, δQ = 0 on these legs)
------------------------------------------------------------------------------
Model dial: per-step fractional ΔA_step = 2.0e-4

C1) J-fixed discharge with dissipation (ΔA > 0)
  ΔM = -0.02949425628
  ΔS > 0 (area increased)    δQ = 0  (by construction on this leg)
  Clausius (leg):  ∫ δQ/T = 0  ≤  ΔS  (✓)

C2) Q-fixed spin with dissipation (ΔA > 0)
  ΔM = -0.07080577931
  ΔS > 0 (area increased)    δQ = 0
  Clausius (leg):  ∫ δQ/T = 0  ≤  ΔS  (✓)

Note on “cycles”: because C1–C2 increase A (horizon entropy), you cannot
...
================================================================================
ONE-LINE TAKEAWAY
Reversible legs saturate ΔM ≈ ∫T dS + ∫Ω dJ + ∫Φ dQ and reach the CR bound;
irreversible legs (ΔA>0, δQ=0) obey Clausius per-leg (0 ≤ ΔS) but cannot close a
...
# MODULE 016 (REPLACED, CLEAN v2): Maxwell Relations & Response Coefficients — Kerr–Newman
...
# Units: geometric (G=c=ħ=k_B=1). State variables: (S, J, Q). Mass scale fixed to M=1 for samples.
...
def cstep_jacobian(func3, rp, a, Q, h=mp.mpf('1e-40')):
    """
    Complex-step Jacobian: returns 3x3 matrix of ∂func/∂(rp,a,Q).
...
def derivatives_at_state(M, a_star, q_star, h=mp.mpf('1e-40')):
    ...
    Ainv = A**-1                                # du/dy (3x3)
    Jint = cjacobian_intensives(rp, a, Q, h=h)  # d(T,Ω,Φ)/du (3x3)
    ...
    # Maxwell residuals
    R1 = abs(dT_dJ - dOm_dS)   # (∂T/∂J)_{S,Q} ?= (∂Ω/∂S)_{J,Q}
    R2 = abs(dT_dQ - dPh_dS)   # (∂T/∂Q)_{S,J} ?= (∂Φ/∂S)_{J,Q}
    R3 = abs(dOm_dQ - dPh_dJ)  # (∂Ω/∂Q)_{S,J} ?= (∂Φ/∂J)_{S,Q}

    # Responses
    T, Om, Phi = intensives_from_u(rp, a, Q)
    CJQ  = T / dT_dS           # heat capacity @ (J,Q)
    chiJ = 1 / dOm_dJ          # (∂J/∂Ω)_{S,Q}
    chiQ = 1 / dPh_dQ          # (∂Q/∂Φ)_{S,J}
...
def print_module_016(states, tol=mp.mpf('1e-55'), h=mp.mpf('1e-40')):
    print("="*79)
    print("MODULE 016 (REPLACED, CLEAN v2): Maxwell Relations & Response Coefficients — KN (geom)")
    print("="*79)
    print("Derivatives via COMPLEX-STEP Jacobians at sub-extremal points (M=1).\n")

    header = ("State                | R1=|∂T/∂J−∂Ω/∂S|   R2=|∂T/∂Q−∂Φ/∂S|   "
              "R3=|∂Ω/∂Q−∂Φ/∂J|   |  C_{J,Q}     χ_J=(∂J/∂Ω)   χ_Q=(∂Q/∂Φ)  | Smarr |LHS−RHS|")
    print(header)
    print("-"*len(header))
    ...
    print("\nLegend:")
    print("  R1, R2, R3  — Maxwell residuals; PASS if all < 1e-55 (strict).")
    print("  C_{J,Q}     — heat capacity at fixed (J,Q) = T / (∂T/∂S)_{J,Q}.")
    print("  χ_J, χ_Q    — susceptibilities χ_J=(∂J/∂Ω)_{S,Q}, χ_Q=(∂Q/∂Φ)_{S,J}.")
    print("  Smarr       — |M − (2TS+2ΩJ+ΦQ)| at the state (should be ~0).")
    print("\nONE-LINE TAKEAWAY")
    print("Complex-step derivatives tighten residuals: Maxwell pairs and Smarr close at ≲1e−55;")
...
# MODULE 017: Potentials, Hessian Symmetry & Local Stability — Kerr–Newman (geom units)
...
def hessian_and_metrics(M, a_star, q_star, h=mp.mpf('1e-40')):
    ...
    Ainv = A**-1                          # du/dy
    JI   = J_int(rp, a, Q, h=h)           # d(T,Ω,Φ)/du
...
def print_module_017(states, tol=mp.mpf('1e-55')):
    print("="*79)
    print("MODULE 017: Hessian Symmetry & Local Stability — Kerr–Newman (geom units)")
    print("="*79)
    print("Hessian H = ∂(T,Ω,Φ)/∂(S,J,Q). Complex-step derivatives; sub-extremal M=1.\n")
    head = ("State                |  r_SJ      r_SQ      r_JQ   |  eig(Hsym)_min  eig2  eig3  | "
            "  C_{J,Q}     χ_J     χ_Q  | Smarr |LHS−RHS|")
    print(head)
    print("-"*len(head))
    for (a_star, q_star, label) in states:
    ...
    print("\nNotes:")
    print("• Maxwell symmetry ⇒ off-diagonal Hessian elements match: H_{SJ}=H_{JS}, etc.")
    print("• Hsym eigenvalues characterize local convexity of M(S,J,Q): signs vary by state/ensemble.")
    print("• C_{J,Q}=T/(∂T/∂S)_{J,Q},  χ_J = (∂J/∂Ω)_{S,Q} = 1/H_{JJ},  χ_Q = (∂Q/∂Φ)_{S,J} = 1/H_{QQ}.")
    print("• Smarr residual should be ≈ 0.")

    print("\nONE-LINE TAKEAWAY")
    print("Hessian off-diagonals are symmetric at ≲1e−55; Smarr closes. Eigenvalues/responders vary")
...
# MODULE 018: Gibbs-like Potential, Dual Hessian & Isopotential Responses — KN (geom units)
# Colab-ready. Complements M016–M017 using the same core relations and complex-step calculus.
...
def J_int(rp, a, Q, h=mp.mpf('1e-40')):
    def f(rp_, a_, Q_):
        return intensives_from_u(rp_, a_, Q_)
    return cstep_jacobian(f, rp, a, Q, h=h)   # d(T,Ω,Φ)/du
...
# H_M = ∂(T,Ω,Φ)/∂(S,J,Q) as in M017.
# G(T,Ω,Φ)=M-TS-ΩJ-ΦQ  ⇒  ∂^2 G/∂(T,Ω,Φ)^2 = - (∂(S,J,Q)/∂(T,Ω,Φ)) = - H_M^{-1}.
def build_hessians(M, a_star, q_star, h=mp.mpf('1e-40')):
    rp, a, Q = state_from_star(M, a_star, q_star)
    A    = J_y(rp, a, Q, h=h)          # dy/du
    Ainv = A**-1                        # du/dy
    JI   = J_int(rp, a, Q, h=h)         # d(T,Ω,Φ)/du
    H_M  = JI * Ainv                    # ∂(T,Ω,Φ)/∂(S,J,Q)
    H_G  = - (H_M**-1)                  # ∂^2 G / ∂(T,Ω,Φ)^2
    ...
    # responses in the (T,Ω,Φ) ensemble from H_G:
    # C_{Ω,Φ} = T*(∂S/∂T)_{Ω,Φ} = -T * (H_G)_{TT}
    # κ_J (isothermal J-suscept.) = (∂J/∂Ω)_{T,Φ} = -(H_G)_{ΩΩ}
    # κ_Q (isothermal Q-suscept.) = (∂Q/∂Φ)_{T,Ω} = -(H_G)_{ΦΦ}
    d = kn_from_u(rp, a, Q)
    T = d["T"]
    C_of = - T * HG_sym[0,0]
    kJ   = - HG_sym[1,1]
...
def print_module_018(states, tol=mp.mpf('1e-55')):
    print("="*79)
    print("MODULE 018: Gibbs-like Potential, Dual Hessian & Isopotential Responses — KN")
    print("="*79)
    print("H_M = ∂(T,Ω,Φ)/∂(S,J,Q),  H_G = ∂²G/∂(T,Ω,Φ)² = - H_M^{-1}. Complex-step; M=1.\n")
    head = ("State                |  maxwell(H_M)  maxwell(H_G) |  eig(H_M)sym (min,mid,max)   | "
            " eig(H_G)sym (min,mid,max)   |   C_{Ω,Φ}      κ_J       κ_Q  | Smarr |LHS−RHS|")
    print(head)
    print("-"*len(head))
    for (a_star, q_star, label) in states:
    ...
    print("\nLegend / notes:")
    print("• Maxwell symmetry in either ensemble: off-diagonals of Hessians match (≲ tol).")
    print("• Duality: H_G = - (H_M)^{-1}; we report eigenvalues of symmetric parts for stability feel.")
    print("• C_{Ω,Φ} = T(∂S/∂T)_{Ω,Φ}  (isopotential heat capacity).")
    print("• κ_J = (∂J/∂Ω)_{T,Φ},  κ_Q = (∂Q/∂Φ)_{T,Ω}  (isothermal susceptibilities).")
    print("• Signs/magnitudes depend on (a*, q*) and encode ensemble-dependent stability.")
    print("\nONE-LINE TAKEAWAY")
...
# ==============================================================================
# MODULE 019 (REPLACED, CLEAN v2): Ensemble Stability Map — dual Hessians on (a*, q*)
# Geometric units (G=c=ħ=k_B=1). Colab-ready.
...
def hessians_at_state(M, a, Q, h=HSTEP):
    # High-prec Jacobians
    JF = jacobian_complex_step_mp(F_SJQ, (M, a, Q), h=h)  # ∂(S,J,Q)/∂(M,a,Q)
    JG = jacobian_complex_step_mp(G_TOmegaPhi, (M, a, Q), h=h)  # ∂(T,Ω,Φ)/∂(M,a,Q)

    # Inverses in high precision
    JF_inv = mp.inverse(JF)
    H_M_mp = JG * JF_inv                 # ∂(T,Ω,Φ)/∂(S,J,Q)
    ...
    # Maxwell symmetry residuals (mp → float for display)
...
# ---------- Report ----------
print("="*110)
print("MODULE 019 (REPLACED, CLEAN v2): Ensemble Stability Map — dual Hessians (geom units, high-prec)")
print("="*110)
...
print("\nResidual ranges on OK states (target ≲ tol):")
print(f"  R1=|∂T/∂J−∂Ω/∂S| : [{fr(R1_min)}, {fr(R1_max)}]")
print(f"  R2=|∂T/∂Q−∂Φ/∂S| : [{fr(R2_min)}, {fr(R2_max)}]")
print(f"  R3=|∂Ω/∂Q−∂Φ/∂J| : [{fr(R3_min)}, {fr(R3_max)}]")
print(f"  Smarr |LHS−RHS|  : [{fr(SM_min)}, {fr(SM_max)}]")
print("\nCSV receipts:")
print(f"  {grid_csv}")
print(f"  {ok_csv}")
print(f"  {fails_csv}")
print("="*110)
print("ONE-LINE TAKEAWAY")
print("Keeping complex-step & inverses in high precision (mpmath) restores ≲1e−80 residuals;")
print("Maxwell & Smarr PASS counts should now reflect the underlying analytic equalities.")
print("="*110)

# ==============================================================================
# MODULE 020 (REPLACED, CLEAN v2): Extremal-Approach Scaling Laws — KN (geom units)
# - Fixes: robust power-law fits (skip zeros/negatives), no log warnings,
#          report #points used, and use the true brink (smallest ε) for lattice check.
# ==============================================================================
...
def fit_power_law_safe(eps_arr, y_arr, min_pos=1e-300, min_pts=3):
    """
    Fit y = C * eps^α on strictly positive y; skip zeros/negatives/NaNs.
...
# -------- Pretty print --------
def f3e(x):
    return "—" if (x is None or (isinstance(x,float) and (np.isnan(x) or np.isinf(x)))) else f"{x:.3e}"
def f4(x):
    return "—" if (x is None or (isinstance(x,float) and (np.isnan(x) or np.isinf(x)))) else f"{x:.4f}"

print("="*110)
print("MODULE 020 (REPLACED, CLEAN v2): Extremal-Approach Scaling Laws — KN (geom units)")
print("="*110)
print(f"Grid: thetas={thetas}, eps in [{eps_list[0]}, {eps_list[-1]}], mp.dps={mp.dps}")
print("\nFits on smallest-ε tail (last 7 points):  y(ε) ~ C * ε^α  (n_used shown per fit)")
print("theta |  α_T   C_T    R2_T  nT ||  α_(1-Ω/Ω_ext)   C_Ω    R2_Ω  nΩ ||  α_(Φ_ext-Φ)   C_Φ    R2_Φ  nΦ || Smarr[min,max]     brink ε   A_lattice_rel")
for r in report:
...
# Notes for any fits that were skipped (e.g., Φ difference identically zero)
notes = []
for r in report:
    for key, label in [("note_T","T"), ("note_O","Ω"), ("note_P","Φ")]:
        if r[key]:
            notes.append(f"θ={r['theta']:.2f}: {label}-fit note: {r[key]}")
if notes:
    print("\nFit notes:")
    for n in notes:
        print("  -", n)
...
print("="*110)
print("ONE-LINE TAKEAWAY")
print("Near extremality along a_*^2+q_*^2 = 1−ε, T_H ~ ε^{1/2} and Ω_H, Φ_H approach their")
...
# ==============================================================================================
# MODULE 021 (REPLACED, CLEAN v2): Near-Extremal Susceptibility Scaling — KN (geom units)
# ==============================================================================================
# Complex-step safe: allow complex in r_± and take real parts for observables.
...
# ---------- Core KN relations (geom units; G=c=ħ=k_B=1) ----------
def r_plus(M, a, Q):
    Δ = M*M - a*a - Q*Q
    return M + mp.sqrt(Δ)  # allow complex during complex-step
...
def jacobian_complex(fun, x, h=mp.mpf('1e-40')):
    ...
    Returns 3x3 mp.matrix of ∂fun/∂x
    """
...
def H_micro(M,a,Q, h=mp.mpf('1e-40')):
    """
    H_M = ∂(T,Ω,Φ)/∂(S,J,Q) at (M,a,Q) via chain rule:
      H_M = (∂(T,Ω,Φ)/∂(M,a,Q)) · (∂(S,J,Q)/∂(M,a,Q))^{-1}
    """
...
for th in thetas:
    ...
    for eps in eps_list:
        ...
        rows.append({
            ...
            "T": float(info["T"]), "Omega": float(info["Ω"]), "Phi": float(info["Φ"])
        })
...
# ---------- Print summary ----------
print("="*110)
print("MODULE 021 (REPLACED, CLEAN v2): Near-Extremal Susceptibility Scaling — KN (geom units)")
print("="*110)
print(f"Grid: thetas={thetas}, eps in [{fe1(eps_list[0])}, {fe1(eps_list[-1])}], mp.dps={mp.dps}")
print("\nFits on smallest-ε tail (last 7 points):  Y(ε) ~ C * ε^α  (n_used shown per fit)")
print("theta |  α_C      C_C      R2_C  nC ||  α_χJ     C_χJ     R2_χJ  nJ ||  α_χQ     C_χQ     R2_χQ  nQ || Smarr[min,max]")
for ft in fits:
    line = (
        f" {ft['theta']:.2f} | "
...
print("\nLegend:")
print("  C_{J,Q}  = T / (∂T/∂S)_{J,Q}  (microcanonical heat capacity)")
print("  χ_J      = (∂J/∂Ω)_{S,Q},   χ_Q = (∂Q/∂Φ)_{S,J}  (microcanonical susceptibilities)")
print("  Smarr    = |M − (2TS + 2ΩJ + ΦQ)| along the scan (should be ~0)")
...
print("\nONE-LINE TAKEAWAY")
print("Near extremality (a_*^2+q_*^2=1−ε), microcanonical responses obey clean power laws in ε;")
print("Smarr and Maxwell symmetries hold to high precision throughout the tail, confirming consistency.")
# ==============================================================================================

# ==============================================================================================================
# MODULE 022 (REPLACED, CLEAN v3): First-Law Path Independence & Legendre Consistency — KN (geom units)
# ==============================================================================================================
# Fix: eliminate direct inversion of H_M that caused "numerically singular" errors.
# - We already compute H_G via stable chain rule:  H_G = - B · A^{-1}.
...
def intensives(M,a,Q):
    T,Ω,Φ = temperature(M,a,Q), omega_H(M,a,Q), phi_H(M,a,Q)
    return (mp.re(T), mp.re(Ω), mp.re(Φ))
...
def F_vec(M,a,Q):  # (T,Ω,Φ)
    T,Ω,Φ = intensives(M,a,Q)
    return mp.matrix([T,Ω,Φ])
...
def safe_inv(Mx, lam=mp.mpf('1e-60')):
    """Ridge-stabilized inverse: (M + λI)^(-1) if plain inverse fails."""
    try:
        return Mx**-1
...
def micro_and_gibbs_hessians(M,a,Q, h=mp.mpf('1e-30')):
    """
    H_M = ∂(T,Ω,Φ)/∂(S,J,Q) = A · B^{-1}, where
      A = ∂(T,Ω,Φ)/∂(M,a,Q),   B = ∂(S,J,Q)/∂(M,a,Q).
    H_G = - B · A^{-1}  (stable, avoids inverting H_M).
    """
...
def line_integral_first_law(path):
    ...
    for k in range(len(path)-1):
        ...
        Tm,Ωm,Φm = intensives(Mm,am,Qm)
        total += Tm*dS + Ωm*dJ + Φm*dQ
    return mp.re(total)
...
def line_integral_gibbs(path):
    ...
    for k in range(len(path)-1):
        M0,a0,Q0 = path[k]
        M1,a1,Q1 = path[k+1]
        T0,Ω0,Φ0 = intensives(M0,a0,Q0)
        T1,Ω1,Φ1 = intensives(M1,a1,Q1)
        dT, dΩ, dΦ = T1-T0, Ω1-Ω0, Φ1-Φ0
        Mm,am,Qm = ( (M0+M1)/2, (a0+a1)/2, (Q0+Q1)/2 )
        Sm,Jm,Qm_r = extensives(Mm,am,Qm)
        total += -(Sm*dT + Jm*dΩ + Qm_r*dΦ)
    return mp.re(total)

def G_value(M,a,Q):
    T,Ω,Φ = intensives(M,a,Q)
    S,J,Qr = extensives(M,a,Q)
    return mp.re(M - T*S - Ω*J - Φ*Qr)

# ---------- Smarr residual ----------
def smarr_abs(M,a,Q):
    T,Ω,Φ = intensives(M,a,Q)
    S,J,Qr = extensives(M,a,Q)
    return abs(M - (2*T*S + 2*Ω*J + Φ*Qr))
...
path_A = [(M0,a0,Q0), (M1,a0,Q0), (M1,a1,Q0), (M1,a1,Q1)]  # M → a → Q
path_B = [(M0,a0,Q0), (M0,a1,Q0), (M1,a1,Q0), (M1,a1,Q1)]  # a → M → Q
...
# ---------- Print summary ----------
print("="*110)
print("MODULE 022 (REPLACED, CLEAN v3): First-Law Path Independence & Legendre Consistency — KN (geom units)")
print("="*110)
print(f"Start (sub-extremal): M=1, a*={f4f(a_star0)}, q*={f4f(q_star0)}")
print(f"End (small step):     ΔM={f3e(dM)}, Δa={f3e(da)}, ΔQ={f3e(dQ)}")

print("\nFirst-law (δM ?= ∫ T dS + Ω dJ + Φ dQ)")
print(f"  Exact ΔM                 = {f3e(dM_exact)}")
print(f"  Path A integral          = {f3e(FL_A)}   |Δ|={f3e(FL_A - dM_exact)}")
print(f"  Path B integral          = {f3e(FL_B)}   |Δ|={f3e(FL_B - dM_exact)}")
print(f"  Path-difference (A−B)    = {f3e(FL_A - FL_B)}")

print("\nGibbs-side (δG ?= ∫(−S dT − J dΩ − Q dΦ),  G=M−TS−ΩJ−ΦQ)")
print(f"  Exact ΔG                 = {f3e(dG_exact)}")
print(f"  Path A integral          = {f3e(GL_A)}   |Δ|={f3e(GL_A - dG_exact)}")
print(f"  Path B integral          = {f3e(GL_B)}   |Δ|={f3e(GL_B - dG_exact)}")
print(f"  Path-difference (A−B)    = {f3e(GL_A - GL_B)}")
...
print("\nSmarr residuals")
print(f"  Start |M−(2TS+2ΩJ+ΦQ)| = {f3e(smarr0)}")
print(f"  End   |M−(2TS+2ΩJ+ΦQ)| = {f3e(smarr1)}")
...
print("\nONE-LINE TAKEAWAY")
print("Stable chain-rule Hessians (H_G = −B·A⁻¹) + ridge inverse for checks avoid singularities;")
print("first-law & Gibbs integrals are path-independent (match ΔM, ΔG), and Maxwell & Legendre hold.")
# ==============================================================================================================

# ============================================================================================================
# MODULE 023 (RE-REPLACED, CLEAN v3): Weinhold & Ruppeiner Geometry — Scalar Curvatures (geom units, Colab-safe)
# ============================================================================================================
# Fix for your last error:
#   • Guarded all Δ comparisons against complex-step numbers: when Δ is complex, we *do not* compare to 0;
#     we just take the complex sqrt. Comparisons are only done for real Δ in runtime (no complex-step).
#   • This removes "TypeError: no ordering relation is defined for complex numbers".
# ============================================================================================================
...
# ---------- Core KN thermodynamics (G=c=ħ=k_B=1) ----------
def _is_complex(z):
...
def r_plus(M, a, Q):
    Δ = M*M - a*a - Q*Q
    if _is_complex(Δ):
        return M + mp.sqrt(Δ)            # complex-step path: allow complex sqrt
    if Δ <= 0:                           # real path: guard against super-extremal
        return mp.nan
    return M + mp.sqrt(Δ)
...
def metrics_at_SJQ(S, J, Q):
    ...
    T,Ω,Φ = intensives(M,a,Qr)
    H = HM_matrix(M,a,Qr)  # ∂(T,Ω,Φ)/∂(S,J,Q)
    ...
    gR = (1/T) * gW
    return dict(M=M, a=a, Q=Qr, T=T, Omega=Ω, Phi=Φ, gW=gW, gR=gR)
...
def christoffel_from_metric(g, dG_dS, dG_dJ, dG_dQ):
    ...
    ginv = safe_inv(g)
    Γ = [[[mp.mpf('0')]*3 for _ in range(3)] for __ in range(3)]  # Γ^m_{ij}
    for m in range(3):
        for i in range(3):
            for j in range(3):
                s = mp.mpf('0')
...
def ricci_scalar(g, Γ, dG_dS, dG_dJ, dG_dQ):
    ...
    for i in range(3):
        for j in range(3):
            ...
            for k in range(3):
                for l in range(3):
                    sum1 += Γ[k][i][j] * Γ[l][k][l]
                    sum2 += Γ[k][i][l] * Γ[l][k][j]
...
def report_state(M, a_star, q_star, dS=mp.mpf('5e-6'), dJ=mp.mpf('5e-6'), dQ=mp.mpf('5e-6'), use_ruppeiner=True):
    ...
    Γ = christoffel_from_metric(g, dG_dS, dG_dJ, dG_dQ)
    Rsc = ricci_scalar(g, Γ, dG_dS, dG_dJ, dG_dQ)

    H = HM_matrix(M,a,Qr)
    R1 = abs(H[0,1]-H[1,0]); R2 = abs(H[0,2]-H[2,0]); R3 = abs(H[1,2]-H[2,1])
    smarr = abs(M - (2*T*S + 2*Ω*J + Φ*Qr))

    def f3e(x): return f"{float(x):.3e}"
    def f4f(x): return f"{float(x):.4f}"

    print("-"*110)
    print(f"State: M=1, a*={f4f(a_star)}, q*={f4f(q_star)}   |   T={f3e(T)}, Ω={f3e(Ω)}, Φ={f3e(Φ)}")
    print(f"Smarr |LHS−RHS| = {f3e(smarr)}   |   Maxwell(H_M): R1={f3e(R1)} R2={f3e(R2)} R3={f3e(R3)}")
    print(f"{'Ruppeiner' if use_ruppeiner else 'Weinhold'} metric g_ij (at y=(S,J,Q)):")
    for i in range(3):
        print("   ["+"  ".join(f3e(g[i,j]) for j in range(3))+"]")
    print(f"Scalar curvature 𝓡_{'R' if use_ruppeiner else 'W'} = {f3e(Rsc)}")
    return dict(S=S,J=J,Q=Qr,T=T,Omega=Ω,Phi=Φ,R=Rsc,smarr=smarr,Rmax=max(R1,R2,R3))

# ---------- Run a small panel ----------
print("="*110)
print("MODULE 023 (RE-REPLACED, CLEAN v3): Weinhold & Ruppeiner Geometry — Scalar Curvatures (geom units)")
print("="*110)
...
print("\nONE-LINE TAKEAWAY")
print("Weinhold metric is the H_M symmetric part; Ruppeiner = Weinhold/T. With complex-step-safe guards on Δ,")
print("Maxwell & Smarr close to high precision and scalar curvatures evaluate cleanly at sub-extremal states.")
# ============================================================================================================

# ============================================================================================================
# MODULE 024 (REPLACED, CLEAN v2): Thermodynamic Geodesics — Ruppeiner Geometry with Shooting (geom units)
# ============================================================================================================
# • Builds Ruppeiner metric g_R(y) on y=(S,J,Q) from H_M; g_R = sym(H_M) / T
# • Computes Christoffels Γ^m_{ij}(y) by finite differences
# • Uses a scalar SHOOTING method to match the desired endpoint and then reports proper lengths:
#     - ℓ_geo : length of the geodesic that *actually hits* the endpoint
#     - ℓ_lin : proper length of the straight (affine) path in (S,J,Q)
# ============================================================================================================
...
def r_plus(M, a, Q):
    Δ = M*M - a*a - Q*Q
    if _is_complex(Δ): return M + mp.sqrt(Δ)
    if Δ <= 0: return mp.nan
    return M + mp.sqrt(Δ)
...
def F_vec(M,a,Q):  # (T,Ω,Φ)
    T,Ω,Φ = intensives(M,a,Q);  return mp.matrix([T,Ω,Φ])
...
def christoffel_from_metric(g, dG_dS, dG_dJ, dG_dQ):
    dG = [dG_dS, dG_dJ, dG_dQ]
    ginv = safe_inv(g)
    Γ = [[[mp.mpf('0')]*3 for _ in range(3)] for __ in range(3)]
    for m in range(3):
        for i in range(3):
            for j in range(3):
                s = mp.mpf('0')
...
def rk4_geodesic_step(y, v, g_at, Γ, dt):
    ...
    k1v = geodesic_rhs(v, Γ); k1y = v

    k2v = geodesic_rhs(add(v,k1v,dt/2), Γ); k2y = add(v,k1v,dt/2)
    k3v = geodesic_rhs(add(v,k2v,dt/2), Γ); k3y = add(v,k2v,dt/2)
    k4v = geodesic_rhs(add(v,k3v,dt),   Γ); k4y = add(v,k3v,dt)
...
def integrate_geodesic(y0, v0_scale, Γ_func, g_func, T=1.0, N=400):
    ...
    L = mp.mpf('0')
    for _ in range(N):
        g_at, Γ = g_func(*y), Γ_func(*y)
        y, v, dℓ = rk4_geodesic_step(y, v, g_at, Γ, dt)
        L += dℓ
    return y, L
...
def shoot_geodesic(y0, y1, steps=400, T=1.0, h=mp.mpf('2e-6'), iters=8):
    ...
    Γ_func = lambda S,J,Q: christoffel_at_point(S,J,Q, h,h,h)[1]
    g_func = lambda S,J,Q: metric_at_point_R(S,J,Q)
    # scalar secant on displacement along dy-direction
    s0 = float(denom)  # first guess: speed ≈ proper length over T
    s1 = 1.1*s0
    def endpoint(s):
        y_end, L = integrate_geodesic(list(y0), s, Γ_func, g_func, T=T, N=steps)
...
def print_state(tag, a_star, q_star):
    ...
    T,Ω,Φ = intensives(1, a_star, q_star)
    print(f"{tag}: a*={a_star:.3f}, q*={q_star:.3f} | S={f6f(S)}, J={f6f(J)}, Q={f6f(Q)} | "
          f"T={f3e(T)}, Ω={f3e(Ω)}, Φ={f3e(Φ)}")

print("="*110)
print("MODULE 024 (REPLACED, CLEAN v2): Thermodynamic Geodesics — Ruppeiner Geometry with Shooting (geom)")
print("="*110)
...
print(" Case A: (0.60,0.30) → (0.55,0.25)")
print(f"   Geodesic length  ℓ_geo = {f6f(shotA['length'])}   (endpoint mis={shotA['mis']:.2e})")
print(f"   Straight length  ℓ_lin = {f6f(L_lin_A)}")
print(f"   ℓ_geo ≤ ℓ_lin ?  {'YES' if shotA['length'] <= L_lin_A else 'NO'}")

print(" Case B: (0.50,0.20) → (0.48,0.18)")
print(f"   Geodesic length  ℓ_geo = {f6f(shotB['length'])}   (endpoint mis={shotB['mis']:.2e})")
print(f"   Straight length  ℓ_lin = {f6f(L_lin_B)}")
print(f"   ℓ_geo ≤ ℓ_lin ?  {'YES' if shotB['length'] <= L_lin_B else 'NO'}")
...
print("Midpoint Smarr/Maxwell checks remain ≲1e−60, backing the Ruppeiner geometry construction.")
# ============================================================================================================

# ============================================================================================================
# MODULE 024 (RE-REPLACED, CLEAN v3): Thermodynamic Geodesics — Ruppeiner Geometry with Vector Shooting (geom)
# ============================================================================================================
# • Ruppeiner metric g_R(y) on y=(S,J,Q) from H_M; g_R = sym(H_M)/T
# • Christoffels Γ^m_{ij}(y) by complex-step finite differences (stable)
# • Geodesic solver with 3D *vector shooting*:
#     - Adjusts initial velocity vector v0 (speed + direction) by Newton steps
#     - Uses ∂y_end/∂v0 (3×3) from finite differences to kill endpoint error
# • Reports:
#     - ℓ_geo for the geodesic that *hits the endpoint*
#     - ℓ_lin for the straight segment’s proper length
# • Midpoint Smarr/Maxwell sanity
# ============================================================================================================
...
def F_vec(M_,a_,Q_):  # (T,Ω,Φ)
    T,Ω,Φ = intensives(M_,a_,Q_);  return mp.matrix([T,Ω,Φ])
...
def integrate_geodesic(y0, v0, T=1.0, N=800, hfd=mp.mpf('2e-6')):
    ...
    L = mp.mpf('0')
    for _ in range(N):
        g_at, Γ = christoffel_at_point(y[0], y[1], y[2], hfd, hfd, hfd)
        y, v, dℓ = rk4_step(y, v, g_at, Γ, dt)
        L += dℓ
    return y, L
...
def shoot_to_target(y0, y1, T=1.0, N=800, maxit=6, tol=1e-10):
    ...
    v0 = [ vi * float(denom) for vi in v0 ]   # speed ≈ proper length over T (T=1)
    ...
    for it in range(maxit):
        ...
        # Build sensitivity J = ∂y_end / ∂v0 via finite differences
        J = mp.matrix(3,3)
        for j in range(3):
            dv = [0.0,0.0,0.0]
        ...
        # Solve J Δv0 = −err  (ridge for safety)
        Δv = - safe_inv(J + mp.mpf('1e-24')*mp.eye(3)) * err
        # damping
        lam = 1.0
        for _ in range(6):
            trial = [v0[i] + lam*float(Δv[i]) for i in range(3)]
            ...
            v0 = [v0[i] + float(Δv[i]) for i in range(3)]  # accept anyway
...
# ---------- Run two cases ----------
print("="*110)
print("MODULE 024 (RE-REPLACED, CLEAN v3): Thermodynamic Geodesics — Ruppeiner Geometry with Vector Shooting (geom)")
print("="*110)
...
print(" Case A: (0.60,0.30) → (0.55,0.25)")
print(f"   Geodesic length  ℓ_geo = {f6f(shotA['length'])}   (endpoint mis={shotA['mis']:.2e}, iters={shotA['iters']})")
print(f"   Straight length  ℓ_lin = {f6f(L_lin_A)}")
print(f"   ℓ_geo ≤ ℓ_lin ?  {'YES' if shotA['length'] <= L_lin_A else 'NO'}")

print(" Case B: (0.50,0.20) → (0.48,0.18)")
print(f"   Geodesic length  ℓ_geo = {f6f(shotB['length'])}   (endpoint mis={shotB['mis']:.2e}, iters={shotB['iters']})")
print(f"   Straight length  ℓ_lin = {f6f(L_lin_B)}")
print(f"   ℓ_geo ≤ ℓ_lin ?  {'YES' if shotB['length'] <= L_lin_B else 'NO'}")
...
print("\nONE-LINE TAKEAWAY")
print("Vector shooting (Newton on the initial velocity) nails the endpoint (mis ≲ 1e−10) and")
print("enables a fair geodesic vs straight-path proper-length comparison in Ruppeiner geometry.")
# ============================================================================================================

# ======================================================================================================
# MODULE 025 (REPLACED, CLEAN v2): Ruppeiner Curvature Near Extremality — Scaling of |R_R| vs ε (geom, M=1)
# ======================================================================================================
# Path: (a_*, q_*) = sqrt(1 - ε) (cosθ, sinθ), ε → 0⁺, θ ∈ {0.0, 0.5, 1.0}
# NOTE (per your request): filenames, printed header text, and CSV path are kept IDENTICAL to your v2:
#   • Prints the same header lines
#   • Saves ONLY: /content/M25_ruppeiner_curvature_scan.csv
#
# Internals upgraded (no interface change):
#   • Robust inverses with adaptive ridge for metric/Hessian inverses
#   • Adaptive finite-difference step for curvature based on |(S,J,Q)|
#   • Complex-step Jacobians, high precision, and safe mp formatting
# ======================================================================================================
...
def HM_matrix(M,a,Q, h=mp.mpf('1e-30')):
    A = jacobian_complex(lambda M_,a_,Q_: F_vec(M_,a_,Q_), (M,a,Q), h=h)  # ∂(T,Ω,Φ)/∂(M,a,Q)
    B = jacobian_complex(lambda M_,a_,Q_: G_vec(M_,a_,Q_), (M,a,Q), h=h)  # ∂(S,J,Q)/∂(M,a,Q)
    return A * safe_inv(B)
...
def metrics_at_SJQ(S, J, Q):
    ...
    for i in range(3):
        for j in range(3):
            gW[i,j] = (H[i,j] + H[j,i])/2
    gR = (1/T)*gW
    return dict(M=M,a=a,Q=Qr,T=T,Omega=Ω,Phi=Φ,gW=gW,gR=gR,H=H,S=S,J=J)
...
def ricci_scalar_Ruppeiner(S,J,Q, h=None):
    ...
    Γ = christoffel_from_metric(g, dS,dJ,dQ)
    ginv = safe_inv(g)
    ...
    ΓS  = _Γ_at(S+h,J,Q);   ΓmS = _Γ_at(S-h,J,Q)
    ΓJ  = _Γ_at(S,J+h,Q);   ΓmJ = _Γ_at(S,J-h,Q)
    ΓQ  = _Γ_at(S,J,Q+h);   ΓmQ = _Γ_at(S,J,Q-h)

    dΓdS = [[[ (ΓS[m][i][j]-ΓmS[m][i][j])/(2*h) for j in range(3)] for i in range(3)] for m in range(3)]
    dΓdJ = [[[ (ΓJ[m][i][j]-ΓmJ[m][i][j])/(2*h) for j in range(3)] for i in range(3)] for m in range(3)]
    dΓdQ = [[[ (ΓQ[m][i][j]-ΓmQ[m][i][j])/(2*h) for j in range(3)] for i in range(3)] for m in range(3)]

    R = mp.mpf('0')
    for i in range(3):
        for j in range(3):
            div = dΓdS[0][i][j] + dΓdJ[1][i][j] + dΓdQ[2][i][j]
            dtrace_i = [dΓdS[0][i][k] + dΓdJ[1][i][k] + dΓdQ[2][i][k] for k in range(3)]
            dj_trace = dtrace_i[j]
            term3 = mp.mpf('0'); term4 = mp.mpf('0')
            for k in range(3):
                tr_kl = sum(Γ[l][k][l] for l in range(3))
                term3 += Γ[k][i][j]*tr_kl
                for l in range(3):
                    term4 += Γ[k][i][l]*Γ[l][j][k]
...
def smarr_and_maxwell_at(a_star,q_star, M=1):
    ...
    T,Ω,Φ = intensives(M, a_star*M, q_star*M)
    H = HM_matrix(M, a_star*M, q_star*M)
    R1 = abs(H[0,1]-H[1,0]); R2 = abs(H[0,2]-H[2,0]); R3 = abs(H[1,2]-H[2,1])
    smarr = abs(M - (2*T*S + 2*Ω*J + Φ*Q))
    return smarr, max(R1,R2,R3)

# ---------- Fit |R| ~ C ε^α on the last k points ----------
...
print("="*110)
print("MODULE 025 (REPLACED, CLEAN v2): Ruppeiner Curvature Near Extremality — Scaling of |R_R| vs ε (geom, M=1)")
print("="*110)
...
for th in thetas:
    ...
    for eps in eps_list:
        ...
        rows.append(dict(theta=float(th), eps=float(eps),
                         ...
                         T=float(T), Omega=float(Ω), Phi=float(Φ),
    ...
    print(f"theta={th:.2f} | α_R={alpha:.4f}  C={C:.3e}  R2={R2:.4f}  n={n}  "
...
print("\nONE-LINE TAKEAWAY")
print("Ruppeiner scalar curvature |R_R| along a_*^2+q_*^2=1−ε shows clean tail scaling in ε;")
print("Smarr & Maxwell stay shut across the sweep, supporting a consistent thermodynamic geometry near extremality.")
# ======================================================================================================

# ======================================================================================================
# MODULE 026: Ensemble Response Sign Map & Singular Loci — Kerr–Newman (geom units, high-prec, no plots)
# ======================================================================================================
# What this does
#   • Scans a grid of sub-extremal states (a*, q*) at fixed M=1
#   • Computes microcanonical responses from H_M = ∂(T,Ω,Φ)/∂(S,J,Q):
#         C_{J,Q} =  T / (∂T/∂S)_{J,Q}      (heat capacity at fixed J,Q)
#         χ_J      = (∂J/∂Ω)_{S,Q} = 1/H_{JJ}   (angular susceptibility)
#         χ_Q      = (∂Q/∂Φ)_{S,J} = 1/H_{QQ}   (electric susceptibility)
#   • Computes Gibbs/isopotential responses from H_G = - H_M^{-1}:
#         C_{Ω,Φ}  =  T * (∂S/∂T)_{Ω,Φ} = - T * (H_M^{-1})_{SS}
#         κ_J      =  (∂J/∂Ω)_{T,Φ}     = - (H_M^{-1})_{JJ}
#         κ_Q      =  (∂Q/∂Φ)_{T,Ω}     = - (H_M^{-1})_{QQ}
#   • Reports:
#       - Counts of OK vs extremal vs singular (ill-conditioned inversion)
#       - PASS rates for Maxwell symmetry & Smarr
#       - Sign tallies (+/−/≈0) for each response
#       - Extremal approach guards; stable complex-step Jacobians
#   • Saves CSV receipts with all per-state numbers
#
# Notes
#   - All computations in geometric units (G=c=ħ=k_B=1); M=1
...
# ---------- print summary ----------
print("="*110)
print("MODULE 026: Ensemble Response Sign Map & Singular Loci — Kerr–Newman (geom units)")
print("="*110)
...
def line_tally(name, t, rng):
    pos,neg,zero,sing = t
    print(f"{name:<12s} signs (+/−/≈0/sing): {pos:4d}/{neg:4d}/{zero:4d}/{sing:4d}   range (finite): [{rng[0]}, {rng[1]}]")

print("Microcanonical responses:")
line_tally("C_{J,Q}",  t_CJQ,  rng_CJQ)
line_tally("χ_J",      t_chiJ, rng_chiJ)
line_tally("χ_Q",      t_chiQ, rng_chiQ)

print("\nIsopotential (Gibbs) responses:")
line_tally("C_{Ω,Φ}",  t_Cph,  rng_Cph)
line_tally("κ_J",      t_kJ,   rng_kJ)
line_tally("κ_Q",      t_kQ,   rng_kQ)

print("\nResidual ranges on OK states:")
print(f"  Smarr |LHS−RHS| : [{rng_sm[0]}, {rng_sm[1]}]")
...
print("are cleanly exposed via H_M and its dual H_G = −H_M^{-1}.")
# ======================================================================================================

# ==============================================================================================
# MODULE 027 (REPLACED, PRO UPGRADE): Meta-Audit — Unified Ledger of Residuals (geom/SI agnostic)
# ==============================================================================================
# Why this upgrade?
#   • Smarter column detection (handles “Smarr”, “Maxwell”, “rel_err”, “Δ…”, “residual”, “mis”, etc.)
#   • Robust numeric coercion (accepts '0E-110', unicode minus '−', mixed text, NaNs)
#   • Log-scale scoring tuned for HEP/GR magnitudes:
#         s_i = clamp( -log10(|err_i|) / L , 0, 1 ), L=80 by default  →  10^-80 ≈ full credit
#   • Per-file and per-module rollups, with counts and log10(min/med/max) of |error|
#   • “Top smallest/largest” tables for quick triage
#   • JSON summary with a single “Unified Ledger Score” ∈ [0,100]
#   • Zero-config: just run; scans /content/*.csv (Colab-friendly)
#
# Outputs:
...
#
# Tips:
#   • Tune LEDGER_L if your target residual scale differs (e.g., L=100 for 10^−100 targets).
#   • Adjust ERR_COL_REGEX if your column naming is unusual.
# ==============================================================================================
...
ERR_COL_REGEX = re.compile(
    ...
    r"dev(iation)?|diff(?!erence.*index)|delta|Δ|error)",  # includes 'Δ' (unicode)
    re.IGNORECASE
)
...
# Score target (orders of magnitude). Reaching 10^(−LEDGER_L) ~ full credit.
LEDGER_L = 80.0
...
def looks_like_error(colname: str) -> bool:
    ...
    if NON_ERR_HINT.search(colname):
        return bool(re.search(r"(smarr|maxwell|res|err|mis|gap|dev|delta|Δ|error)", colname, re.IGNORECASE))
...
def to_float_series(series: pd.Series) -> pd.Series:
    ...
    def _coerce(x):
        ...
        s = str(x).strip().replace('−', '-')  # unicode minus → ASCII
...
def score_from_errors(abs_errors: np.ndarray, L: float = LEDGER_L) -> float:
    ...
    s = -safe_log10_abs(abs_errors) / L     # larger −log10 → closer to 1
    s = np.clip(s, 0.0, 1.0)
...
if not csv_paths:
    print("MODULE 027 (UPGRADED): No CSV receipts found under /content — nothing to audit.")
else:
    ledger_rows = []
    ...
    # ---------- Console report ----------
    print("="*106)
    print("MODULE 027 (REPLACED, PRO UPGRADE): Meta-Audit — Unified Ledger of Residuals")
    print("="*106)
    ...
    print("-"*106)
    print("Tip: Set LEDGER_L=100 if your target residuals are ~10^−100 and you want full credit at that scale.")
...
#      so r_±, T, Ω, Φ work with mpmath.mpc during Jacobian evaluation.
# ==========================================================================================================
...
def r_plus(M, a, Q):
    Δ = M*M - a*a - Q*Q
    # For complex-step, allow complex Δ; for real negative Δ (super/extremal) return NaN.
    if _is_complex(Δ):
        return M + mp.sqrt(Δ)
    if Δ <= 0:
        return mp.nan
...
def T_of(M, a, Q):
    # Use (r_+ - r_-) / (4π (r_+^2 + a^2)) — complex-step safe.
    rp = r_plus(M,a,Q); rm = r_minus(M,a,Q)
...
def smarr_residual(M, a, Q):
    T,Ω,Φ,S,J = T_of(M,a,Q), Ω_of(M,a,Q), Φ_of(M,a,Q), S_of(M,a,Q), J_of(M,a,Q)
    # If any NaN (near extremal real-only guard), treat as 0 residual
    if any(mp.isnan(v) for v in [T,Ω,Φ,S,J]):
        return mp.mpf('0')
    return abs(M - (2*T*S + 2*Ω*J + Φ*Q))

# ---------- Complex-step Jacobians ----------
def cs_jacobian_vec(fvec, x, h=H_CS):
    """
    Complex-step Jacobian for 3→3 maps. fvec returns (f1,f2,f3); x=(M,a,Q).
...
def cs_jacobian_general(fvec, x, h=H_CS):
    """
    Complex-step Jacobian for generic m×n maps.
...
def thermo_vec(M,a,Q):
    return (T_of(M,a,Q), Ω_of(M,a,Q), Φ_of(M,a,Q))

def state_vec(M,a,Q):
    return (S_of(M,a,Q), J_of(M,a,Q), Q)

# ---------- Chain rule: H_M = d(T,Ω,Φ)/d(S,J,Q) ----------
def HM_chainrule(M,a,Q, ridge=RIDGE):
    Jto, _ = cs_jacobian_vec(thermo_vec, (M,a,Q))   # 3x3  d(T,Ω,Φ)/d(M,a,Q)
...
def christoffels_and_R(g, y0, h=H_FD, fmetric=None):
    ...
    # helper to rebuild Γ at a nearby point (for Γ derivatives)
    def Γ_at(y):
        g_here = fmetric(y)
        invg_here = g_here**-1
        pg_here = [ (partial_g(y,i,+1) - partial_g(y,i,-1))/(2*h) for i in range(3) ]
        Γh = [[[mp.mpf('0')]*3 for _ in range(3)] for __ in range(3)]
        for k in range(3):
            for i in range(3):
                for j in range(3):
                    s = mp.mpf('0')
    ...
    # ∂_j Γ by central difference
    def dΓ(j_idx):
        y_plus  = list(y0); y_plus[j_idx]  += h
        y_minus = list(y0); y_minus[j_idx] -= h
        Γp = Γ_at(tuple(y_plus))
        Γm = Γ_at(tuple(y_minus))
        out = [[[mp.mpf('0')]*3 for _ in range(3)] for __ in range(3)]
        for m in range(3):
            for i in range(3):
                for k in range(3):
                    out[m][i][k] = (Γp[m][i][k] - Γm[m][i][k])/(2*h)
        return out

    dΓ_list = [dΓ(0), dΓ(1), dΓ(2)]

    # Ricci (approximate with symmetric contraction of ∂Γ terms)
    R = [[mp.mpf('0')]*3 for _ in range(3)]
    for i in range(3):
        for k in range(3):
            s = mp.mpf('0')
            for m in range(3):
                # ∑_j (∂_j Γ^m_{ik} - ∂_k Γ^m_{ij})
                s_j = mp.mpf('0')
                for j in range(3):
                    s_j += dΓ_list[j][m][i][k] - dΓ_list[k][m][i][j]
                s += s_j
            R[i][k] = s
...
# ---------- M19’ recompute (dual Hessians over grid) ----------
...
def run_M25_fixed(thetas=(0.0,0.5,1.0), eps_list=None, out_prefix="/content/M25_fixed"):
    ...
    if os.path.exists("/content/M25_ruppeiner_curvature_scan.csv"):
        ...
        print("[M25′] BEFORE |R_R| stats:",
              f"min={o_min:.3e}, median={o_med:.3e}, max={o_max:.3e}")
    if len(df):
        print("[M25′]  AFTER |R_R| stats:",
              f"min={df['R_abs'].min():.3e}, median={df['R_abs'].median():.3e}, max={df['R_abs'].max():.3e}")
    return df

# ---------- Run both re-computes ----------
print("="*106)
print("MODULE 027-FIXKIT: Recomputing Hessians (M19′) & Ruppeiner Curvature (M25′) with robust numerics")
...
# ==============================================================================================================
# MODULE 028 (REPLACED, CLEAN v2): Unified Meta-Audit — Ledger, Integrity, Anomalies, Verdict
# ==============================================================================================================
# Goal
#   Ingest *everything* under /content — CSV/JSON/TXT/LOG — extract residual-like numerics (Smarr, Maxwell,
#   residuals, rel_err, Δ, etc.), plus text flags; compute:
#     • Global unified score (0..100) with target 10^-L (default L=80)
#     • Per-module integrity table (median quality, score, counts, flags)
#     • Top anomalies/outliers (worst/best numerics; textual flags)
...
#   • Ignores *quality columns* named like "score", "ledger_score", "file_score" so they do NOT get treated as errors.
#   • Suppresses plain "nan/inf" flags unless the line/field also matches an error-ish token (Smarr, Maxwell, err, resid, Δ, …).
#   • Robust numeric parsing (supports unicode minus '−' and "0E-110" style numbers).
#   • No pandas deprecation warnings; no formatting errors on ints/floats.
#   • Colab/Posix friendly; safe if directory is empty.
# ==============================================================================================================
...
LEDGER_L    = 80.0                 # full-credit target: 10^(−L)
EPS_FLOOR   = 1e-300               # avoid log10(0)
...
ERR_TOKENS  = re.compile(
    ...
    r"diff(?!erence.*index)|delta|Δ|sigma|σ|codata|curvature|curv|lattice|snap|"
...
def text_flags(s: str):
    # Only surface plain 'nan'/'inf' when an error-ish token is present (Smarr/Maxwell/err/…)
...
def safe_q_from_err(vals: np.ndarray) -> np.ndarray:
    """Quality metric q = −log10(|err|). Higher is better."""
...
def robust_anomaly_mask(q: np.ndarray):
    ...
    # Consider anomalous if q < med − 3*MAD
...
if not paths:
    print("MODULE 028: No receipts found under /content — nothing to audit.")
else:
    for path in paths:
        module = extract_module_id(path)
...
if rec_df.empty and flags_df.empty:
    ...
    # ------------------------ Unified Print Block ------------------------
    print("="*118)
    print("MODULE 028: Unified Meta-Audit — Ledger, Integrity, Anomalies, Verdict")
    print("="*118)
    ...
    if not math.isnan(unified_score):
        ...
        if not math.isnan(best_q) and not math.isnan(worst_q):
            print(f"Quality q = −log10(|err|): best={best_q:.1f}  worst={worst_q:.1f}  (higher is better)")
    else:
...
# ==========================================================================================================
# MODULE 029: Cross-Ensemble Phase Portrait & Stability Sign Topology — Kerr–Newman (geom units)
# ==========================================================================================================
# Goal
#   Fuse the dual-ensemble receipts (microcanonical from M19, sign map from M26 if present) and produce a
#   text/CSV phase portrait over (a*, q*) that labels each state by stability signs and highlights “boundary”
...
#   - Ensemble “stability verdicts” (microcanonical vs isopotential)
#   - Boundary statistics and a few sample boundary coordinates
#
# Notes
#   - No plots; text + CSV only, Colab-friendly.
#   - Robust to missing columns; if some responses aren’t available, module still runs and reports what it can.
# ==========================================================================================================
...
if df19.empty and df26.empty:
    print("="*110)
    print("MODULE 029: No usable M19/M26 receipts found under /content — nothing to map.")
    print("="*110)
else:
    # ---------- Harmonize + derive ----------
    ...
    # Known response columns we’ll use if present:
    RESP_COLS = dict(
        C_JQ      = "C_JQ",
        chi_J     = "chi_J",
        chi_Q     = "chi_Q",
    ...
    # Thresholds
    NEAR0 = 1e-120      # numerical “≈0” band for responses
    PASS_TOL = 1e-80    # tight closure for Smarr/Maxwell

    def sign_bucket(x):
        # return +1, -1, 0 (≈0), or np.nan
        try:
    ...
    # Compute sign buckets for each ensemble’s key responses (when available)
    if not df19.empty:
        df = df19.copy()
        ...
        # Ensemble “stability verdicts” (very simple sign rules; informative, not prescriptive)
        # Microcanonical: heat capacity positive & susceptibilities positive
        df["mc_stable"] = (
            (df["sgn_CJQ"]==1) & (df["sgn_chiJ"]==1) & (df["sgn_chiQ"]==1)
        )
        # Gibbs/isopotential: C_{Ω,Φ} positive & κ_J, κ_Q positive
        ...
        # If M26 exists, overlay (optional): we won’t overwrite numerics,
        ...
        # This requires the (a*, q*) to be on a grid; we’ll round to 2 decimals (as in M19).
        def _round2(x):
        ...
        summary = dict(
            ...
            notes="Signs: +1/−1/0≈; Stability: microcanonical=(C_JQ, χ_J, χ_Q > 0), "
                  "Gibbs=(C_{Ω,Φ}, κ_J, κ_Q > 0). Boundaries mark sign flips in the 4-neighborhood."
        )
        ...
        # ---------- Print report ----------
        print("="*110)
        print("MODULE 029: Cross-Ensemble Phase Portrait & Stability Sign Topology — KN (geom units)")
        print("="*110)
        print(f"Inputs:")
        if src19: print(f"  • M19 responses: {src19}")
        if src26: print(f"  • M26 overlay  : {src26}")
        print("\nCounts & closures:")
        ...
        if "r_SJ" in df or "r_SQ" in df or "r_JQ" in df or "smarr" in df:
            ...
            if "r_SJ" in df: print(f"    R1=|∂T/∂J−∂Ω/∂S| : {_rstr(R1_rng)}")
            if "r_SQ" in df: print(f"    R2=|∂T/∂Q−∂Φ/∂S| : {_rstr(R2_rng)}")
            if "r_JQ" in df: print(f"    R3=|∂Ω/∂Q−∂Φ/∂J| : {_rstr(R3_rng)}")
            if "smarr" in df: print(f"    Smarr |LHS−RHS|   : {_rstr(SM_rng)}")
        ...
        if df_boundary.shape[0]:
            ...
            for r in sample:
                print(f"    (a*={r['a_star']:.2f}, q*={r['q_star']:.2f}) — flips in [{r['which']}]")
        ...
        # Only M26 present — print a concise note and export M26 as portrait-ish data.
...
# ==========================================================================================================
# MODULE 030: Release Packager + Headliner ASCII Summary — Kerr–Newman Project (geom units, Colab-ready)
# ==========================================================================================================
# What this does:
#   • Scans /content for all module artifacts (CSV/JSON/TXT/LOG).
#   • Pulls integrity metrics from M28 outputs if present; otherwise derives soft metrics from receipts.
#   • Computes per-module file counts, numeric-record counts, and (when possible) Smarr/Maxwell pass counts.
#   • Renders a big ASCII "headliner" table to stdout and writes it to /content/M30_release_summary.txt.
#   • Produces a release ZIP at /content/M30_release_pack.zip with a MANIFEST.json and all artifacts.
#
